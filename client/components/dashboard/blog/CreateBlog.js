...
import { useCreateBlogMutation } from "../../../src/features/blog/blogAPI";
...
export default function CreateBlog() {
  ...
  const handleTableofContentChange = (e, index) => {
    e.preventDefault();
    const newTableofContent = [...tableofContent];
    newTableofContent[index][e.target.name] = e.target.value;
    setTableofContent(newTableofContent);
  };
  const handleTableofContentsChange = (e, index) => {
    const newTableofContent = [...tableofContent];
    newTableofContent[index].content = e;
    setTableofContent(newTableofContent);
  };
  ...
  return (
    <section className="w-full h-full flex justify-center ">
      {isLoading ? (
        ...
        <div className="border h-full w-[80%] bg-white rounded-lg p-6 overflow-y-scroll flex flex-col gap-y-2">
          <form
